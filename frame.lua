...
local round = function(n) return math.floor(n * 1e5 + .5) / 1e5; end

function Frame:SavePosition()
    local point, parent, _, x, y = UIFrame:GetPoint();

    defaults.framePosition = string.format('%s\031%s\031%d\031%d', point,
                                           'UIParent', round(x), round(y));
end
...
function Frame:Init()
    UIFrame = core.Override.CreateFrameA(nil, "Button", "ListLooterLootFrame",
                                         UIParent)
    UIFrame:SetPoint("TOPLEFT", UIParent, "TOPLEFT", 2 * defaults.delta,
                     -2 * defaults.delta);
    UIFrame:SetSize(1, 1);
...
function Frame:GetEmptyItem() return defaults.item; end
...
function Frame:AddItem(...)
    ...
    if (item.currencyID) then
        item.lootName, item.lootIcon, item.lootQuantity, item.lootQuality =
            CurrencyContainerUtil.GetCurrencyContainerInfo(item.currencyID,
                                                           item.lootQuantity,
                                                           item.lootName,
                                                           item.lootIcon,
                                                           item.lootQuality);
...
function Frame:CreateItemFrame(masqueGroup)
    ...
    local frame = core.Override.CreateFrameA(nil, "Button",
                                             "ListLooterLootFrameItem" .. posId,
                                             UIFrame);
    ...
    local iconFrame = core.Override.CreateFrameA(nil, "Button",
                                                 "ListLooterLootFrameItemIcon",
                                                 frame);
...
function Frame:GetVisibleListItems()
    ...
    for i = 1, table.getn(UIFrame.Items) do
        ...
        if (frame:IsShown()) then result = result + 1; end
    end
    return result;
end

function Frame:AnchorItemsFrames()
    local frameSize = math.max(ListLooterDB.frame.iconSize,
    ...
    if Masque then
        frameSize = math.max(ListLooterDB.frame.iconSize + 4,
                             ListLooterDB.frame.fontSizeItem);
    end
...
function Frame:CloseLootFrame()
    ...
    for i = 1, table.getn(UIFrame.Items) do UIFrame.Items[i]:Hide(); end
...
function Frame:UpdateSettings()
    for i = 1, table.getn(UIFrame.Items) do
        ...
        frame.name:SetFont(core.FontProvider:GetFontName(),
                           ListLooterDB.frame.fontSizeItem, nil);
        frame.count:SetFont(core.FontProvider:GetFontName(),
                            ListLooterDB.frame.fontSizeCount, nil);

        frame:SetHeight(ListLooterDB.frame.iconSize);

        if Masque then
            frame.iconFrame:SetSize(ListLooterDB.frame.iconSize + 4,
                                    ListLooterDB.frame.iconSize + 4);
            if UIFrame.masqueGroup then
                UIFrame.masqueGroup:ReSkin(true);
            end
        else
            frame.iconFrame:SetSize(ListLooterDB.frame.iconSize,
                                    ListLooterDB.frame.iconSize);
        end
        frame.iconFrame:SetSize(ListLooterDB.frame.iconSize,
                                ListLooterDB.frame.iconSize);

        frame.quest:SetSize(ListLooterDB.frame.iconSize * .8,
                            ListLooterDB.frame.iconSize * .8);
