...
core.commands = {
    ...
    ["font"] = function() core.FontProvider.Init() end,
    ["help"] = function()
        print(" ");
...
function core:init(event, name)
    ...
    if (event == "LOOT_OPENED") then core:Loot(); end

    if (event == "LOOT_CLOSED") then
        local config = core.Config:GetSettings();
        if (config.isLootFrame) then core.Frame.CloseLootFrame(); end
    end
...
function AddTestItem(id, index)
    local itemName, _, itemRarity, _, _, _, _, _, _, itemTexture = GetItemInfo(
                                                                       id)
    if (itemName == nil) then
        core:Debug("Item cache not ready...");
    else
        core.Frame.AddItem("table", index, itemTexture, itemName, 1, nil,
                           itemRarity, false, false, false, true);
    end
end
...
function core:Loot()
    ...
    for i = 1, numLootItems, 1 do
        ...
        item.index = i;
        item.lootIcon, item.lootName, item.lootQuantity, item.currencyID, item.lootQuality, item.locked, item.isQuestItem, item.questID, item.isActive =
            GetLootSlotInfo(i);
        ...
        if config.isLootEnable == true then
            ...
            if (config.isCurrency == true and not ifLooted) then
                ...
                if item.currencyID ~= nil and lootSlotType ~= LOOT_SLOT_MONEY then
                    ...
                    if (info.maxQuantity == nil or info.maxQuantity == 0 or
                        info.maxQuantity >= (info.quantity + item.lootQuantity)) then
                        core:Debug(
                            "Core:Loot(): Loot money or currency. Slot: " .. i ..
                                " " .. item.lootIcon .. " " .. item.lootName);
                        LootSlot(i);
                        ifLooted = true;
                    end
                elseif item.currencyID == nil and lootSlotType ==
                    LOOT_SLOT_MONEY then
                    core:Debug("Core:Loot(): Loot money or currency. Slot: " ..
                                   i .. " " .. item.lootIcon .. " " ..
                                   item.lootName);
                    LootSlot(i);
                    ifLooted = true;
            ...
            if (config.isQuestItem == true and not ifLooted) then
                if item.isQuestItem == true then
                    core:Debug("Core:Loot(): Loot quest item. Slot: " .. i ..
                                   " " .. item.lootIcon .. " " .. item.lootName);
                    LootSlot(i);
                    ifLooted = true;
                end
            ...
            if (config.isFishingLoot == true and not ifLooted) then
                ...
                if (isFishingLoot) then
                    core:Debug("Core:Loot(): Loot fishing loot. Slot: " .. i ..
                                   " " .. item.lootIcon .. " " .. item.lootName);
                    LootSlot(i);
                    ifLooted = true;
                end
            ...
            if (not ifLooted) then
                for c = 1, table.getn(db), 1 do
                    if itemLink ~= nil then
                        ...
                        if db[c] == Id then
                            core:Debug("Core:Loot(): Loot item. Slot: " .. i ..
                                           " " .. item.lootIcon .. " " ..
                                           item.lootName);
                            LootSlot(i);
                            ifLooted = true;
        ...
        if (config.isLootFrame and not ifLooted) then
            core.Frame.AddItem("table", item.index, item.lootIcon,
                               item.lootName, item.lootQuantity,
                               item.currencyID, item.lootQuality, item.locked,
                               item.isQuestItem, item.questID, item.isActive);
        end
    end
    ...
    if (config.isLootFrame) then core.Frame.ShowLootList(); end
end
